...
import { getDemoEmployeeByEmail, serializeDemoEmployee } from "../demo/demo-store";
...
export async function login(input: LoginInput) {
  if (config.demoMode) {
    const employee = getDemoEmployeeByEmail(input.email);
    ...
    const token = jwt.sign(
      ...
      config.jwtSecret,
      { subject: employee._id, expiresIn: config.jwtExpiresIn } as SignOptions
    );

    return {
      token,
      user: serializeDemoEmployee(employee)
    };
  }
